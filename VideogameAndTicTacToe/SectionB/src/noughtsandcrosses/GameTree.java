		...
		if (!board.isFinished()) {
			...
			for (int pos = 1; pos <= 9; pos++) {
				if (board.getMark(pos) == Board.EMPTY) {
					Board boardCopy = board.makeCopy();
					boardCopy.setMark(pos, turn);
					boardCopy.setLastMarkPosition(pos);
					GameTreeNode newNode = new GameTreeNode(boardCopy);
					node.getChildren().add(node.numberOfChildren() + 1, newNode);
					expandTree(newNode);
				}
			}
		}
